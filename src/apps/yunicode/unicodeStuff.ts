...
export const alphabetsDB = {
  "normal": {
    ...
    "srevnela":
      "ꓯꓭↃꓷƎℲ⅁ɥıᒋꓘ⅂WuOꓒÒꓤSꓕꓵ𐌡MX⅄Z ɐqɔpǝɟƃɥıɾʞןɯuodbɹsʇnʌʍxʎz 0Ɩ↊↋Һꞔ9∠86",
  },
  ...
  "serif": {
    ...
    "gras-italique":
      "𝑨𝑩𝑪𝑫𝑬𝑭𝑮𝑯𝑰𝑱𝑲𝑳𝑴𝑵𝑶𝑷𝑸𝑹𝑺𝑻𝑼𝑽𝑾𝑿𝒀𝒁 𝒂𝒃𝒄𝒅𝒆𝒇𝒈𝒉𝒊𝒋𝒌𝒍𝒎𝒏𝒐𝒑𝒒𝒓𝒔𝒕𝒖𝒗𝒘𝒙𝒚𝒛 𝟎𝟏𝟐𝟑𝟒𝟓𝟔𝟕𝟖𝟗",
  },

  "sans-serif": {
    "normal":
      "𝖠𝖡𝖢𝖣𝖤𝖥𝖦𝖧𝖨𝖩𝖪𝖫𝖬𝖭𝖮𝖯𝖰𝖱𝖲𝖳𝖴𝖵𝖶𝖷𝖸𝖹 𝖺𝖻𝖼𝖽𝖾𝖿𝗀𝗁𝗂𝗃𝗄𝗅𝗆𝗇𝗈𝗉𝗊𝗋𝗌𝗍𝗎𝗏𝗐𝗑𝗒𝗓 𝟢𝟣𝟤𝟥𝟦𝟧𝟨𝟩𝟪𝟫",
    ...
    "gras-italique":
      "𝘼𝘽𝘾𝘿𝙀𝙁𝙂𝙃𝙄𝙅𝙆𝙇𝙈𝙉𝙊𝙋𝙌𝙍𝙎𝙏𝙐𝙑𝙒𝙓𝙔𝙕 𝙖𝙗𝙘𝙙𝙚𝙛𝙜𝙝𝙞𝙟𝙠𝙡𝙢𝙣𝙤𝙥𝙦𝙧𝙨𝙩𝙪𝙫𝙬𝙭𝙮𝙯 𝟎𝟏𝟐𝟑𝟒𝟓𝟔𝟕𝟖𝟗",
  },
  ...
  "monospace": {
    "normal":
      "𝙰𝙱𝙲𝙳𝙴𝙵𝙶𝙷𝙸𝙹𝙺𝙻𝙼𝙽𝙾𝙿𝚀𝚁𝚂𝚃𝚄𝚅𝚆𝚇𝚈𝚉 𝚊𝚋𝚌𝚍𝚎𝚏𝚐𝚑𝚒𝚓𝚔𝚕𝚖𝚗𝚘𝚙𝚚𝚛𝚜𝚝𝚞𝚟𝚠𝚡𝚢𝚣 𝟶𝟷𝟸𝟹𝟺𝟻𝟼𝟽𝟾𝟿 ？！＇，．",
    "large":
...
const normal = alphabetsDB["normal"]["normal"];

export const modifiersDB = {
  "barré": {
...
export type ModifierVariant<F extends Font = Font> = KeysOfUnion<
  typeof alphabetsDB[F]
>;
...
function parseModifier(mod: string) {
  if (mod.startsWith("U+")) {
    const int = parseInt(mod.substring(2), 16);
...
export default function getVariant(text: string, params: WritingParams) {
  const font = params.alphabet.font;
  const variant = params.alphabet.variant;
  const modifiers = params.modifiers;
  const verlan = params.verlan;

  let list = text.split("");
  const sufix = modifiers.map((e) => parseModifier(e)).join("");
  if (verlan) list = list.reverse();

  let alphabet = Array.from(normal);

  const variants = alphabetsDB[font];
  if (Object.keys(variants).includes(variant)) {
    alphabet = Array.from(variants[variant as keyof typeof variants]);
  }

  list = list.map((char) => {
    const id = normal.indexOf(char);
    const cr = alphabet[id];
    return cr || char;
  });
